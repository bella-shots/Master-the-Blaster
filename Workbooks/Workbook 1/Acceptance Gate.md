...
| AC4 | Cost invariant is preserved. | Scan dependencies, hosting, runtime, APIs and configuration and observe billing state. | Dependency inventory + verified billing state. | STOP/redesign (HG-02). | P0 | BLOCKED |
| AC5 | Admin vs standard-user authorization is explicit and enforced. | Run actual/emulator Security Rules tests. | Allowed/denied test results. | Fix security architecture (HG-05). | P0 | BLOCKED |
| AC6 | Drive/Gmail/Auth dependencies and human gates are explicit and tested. | Review actual proof records and gates. | Integration test records + exact human gates. | Add gate (HG-03, HG-04, HG-05). | P0 | BLOCKED |
...
| AC11 | Cloud Run/App Hosting are not mandatory production dependencies under ₹0 invariant. | Verify architecture and actual dependency graph. | Explicit production graph + no-billing evidence. | Reject/stop (HG-02). | P0 | BLOCKED |
| AC12 | ZBA-01..ZBA-07 are all PASS. | Verify each ZBA row independently before deriving ZBA-07. | Seven evidence-backed PASS results. | Do not freeze/proceed (HG-02..05). | P0 | BLOCKED |
...
| AC15 | Completion claim is evidence-based. | Independently enumerate every current PASS and inspect its evidence registry row/artifact. | Complete PASS-by-PASS audit. | Keep incomplete and correct evidence. | P0 | PASS |
